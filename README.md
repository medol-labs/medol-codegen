...
Frontend application boundaries are modeled separately from backend
deployments. A backend deployment describes where context APIs run; a frontend
application describes which contexts or slices are visible in one generated
console. For example:
...
Generate a single console with:
...
```bash
CODEGEN_FRONTEND_APP=FederationLearningParticipantConsole gen /opt/codegen/.generator/app/ --generator refine --generator-type all
```

When a frontend application is selected, generated resources, routes, pages,
i18n, and TypeScript command/value-type models are projected to that frontend
application. Each projected resource still uses the backend data provider
declared by `from <BackendDeployment>` when present, and otherwise falls back
to the context's backend deployment. When no `--output` is supplied, the refine
generator writes the selected frontend application into a project directory
derived from its name, for example `FederationLearningParticipantConsole`
becomes `federation-learning-participant-console`. Pass `--output` to override
that directory explicitly. This keeps frontend granularity and
backend service granularity intentionally independent.
...
When the model declares multiple `frontend` applications, generate each frontend
into its own project directory before packaging:

```bash
gen /opt/codegen/.generator/app/ --generator refine --generator-type all \
  --frontend-app FederationLearningConsole

gen /opt/codegen/.generator/app/ --generator refine --generator-type all \
  --frontend-app FederationLearningParticipantConsole
```
...
orchestrator for backend, frontend, and infrastructure images. With multiple
frontend applications, operations creates one frontend application, image,
archive, gateway route, Compose service, and Kubernetes deployment per modeled
frontend. The first frontend is mounted at `/`; later frontends default to
...
node operations/dev/images.mjs build --service federation-learning-participant-console
...
APISIX is generated in standalone mode with declarative YAML and Admin API disabled. Docker Compose is the primary runnable target and includes generated backend services, generated frontend consoles, runtime dependencies such as PostgreSQL and UMA DB, optional/profiled Axon Server, optional Redis, volumes, networks, environment placeholders, health checks, and service startup dependencies.
